...
8. **Pagamento** — banner upsell PDF a 9,90 EUR. Prezzo verticale (`price-big` 9,90€ + nota "pagamento unico • PDF via email, pronto in poche ore"), divider "Cosa c'è nel tuo PDF", lista benefici con BONUS make-up, CTA "Ricevi l'analisi · 9,90€" e nota "Pagamento sicuro con Stripe". Checkout Stripe via `/api/create-checkout`. _(Nota: il redesign del 24/05 — price-bar orizzontale + blocco social-proof cumulativo "Unisciti a N donne" con avatar + endpoint `/api/social-proof` — è stato annullato per isolare un calo di conversione; il banner è tornato a questa versione pre-18-maggio.)_
...
| `/sconto` | GET | Pagina sconto reminder. Params: `?email=...&name=...&season=...`. Redirect immediato a Stripe checkout a **7€ sempre** (metadata `discount: abandoned_cart_20`), senza scadenza. _(La scadenza a 4 giorni introdotta il 18/05 è stata rimossa il 25/06: lo sconto non scade più.)_ |
...
| reminder_sent_at | TEXT | Timestamp invio reminder 1 (NULL se mai inviato) |
...
- Email abandoned cart reminder — **una sola mail** (logica in [src/lib/abandoned-cart.ts](src/lib/abandoned-cart.ts)). Timeline rispetto al momento del subquiz abbandonato:
  - T+15min → **Mail 1** (unica mail di recupero)
  - **Mail 1** — Template lunga e personale (presentazione di Veronica, value prop, sconto a 7€). Schedulata in memoria con `setTimeout` 15min via `scheduleAbandonedCartReminder` (chiamata da `/api/subquiz-upload` sul primo INSERT). Guard idempotente su `reminder_sent`/`paid` per evitare doppi invii.
  - _**Nota storica:** dal 18/05 al 25/06 era attiva una **sequenza a 3 mail** (mail 1 +15min, mail 2 +24h, mail 3 +72h, con lazy polling). È stata rimossa il 25/06 tornando a 1 sola mail (richiesta di Veronica). Le colonne `reminder2/3_*` restano in tabella per lo storico e per il tracking aperture, ma non vengono più scritte._
  - **Paid tracking** — quando l'utente paga (sia via `/sconto` a 7€ sia via flusso normale a 9,90€), [src/app/api/upload/route.ts](src/app/api/upload/route.ts) chiama `markSubquizPaidByEmail(email)` che setta `paid = 1` su tutte le submission di quella email. Questo "spegne" il reminder pending: il guard `paid = 1` nel `setTimeout` lo esclude.
  - **Open rate tracking — DISATTIVATO il 25/06** — il pixel invisibile 1×1 nella mail (`getOpenPixel`) è stato **rimosso**: un'immagine remota nascosta è un segnale antispam e stiamo lavorando sulla deliverability, quindi la mail 1 ora non contiene più pixel di tracking. L'infrastruttura lato server resta in piedi ma inerte: la route [`/api/email/open`](src/app/api/email/open/route.ts), le funzioni `markReminderOpened`/`getReminderOpenStats` ([db.ts](src/lib/db.ts)) e il riquadro "Open rate mail post-subquiz" nell'admin ([AdminDashboard.tsx](src/app/admin/AdminDashboard.tsx)) continuano a mostrare lo **storico** raccolto fino al 25/06, ma non arrivano nuove aperture (le mail inviate dopo non hanno pixel). _(Se in futuro si vuole rimuovere del tutto anche route/admin/colonne, è un cleanup a parte.)_
